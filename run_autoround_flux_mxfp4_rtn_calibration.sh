...
if [[ $# -lt 3 || $# -gt 4 ]]; then
  echo "Usage: $0 <physical_gpu> <nsamples> <num_inference_steps> [max_calibration_calls]" >&2
  exit 2
fi
...
TAG="n${NSAMPLES}-s${STEPS}-k${MAX_CALLS}"
...
echo "GPU=$GPU nsamples=$NSAMPLES steps=$STEPS max_calibration_calls=$MAX_CALLS"
echo "Output: $OUT"
echo "Log: $LOG"
...
"$PY" -u -m auto_round \
  ...
  --svdquant_smooth_max_calibration_calls "$MAX_CALLS" \
